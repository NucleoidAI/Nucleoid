...
# ============================================================================
# SUMMARY: API Tests (All Skipped)
# ============================================================================
"""
⏭️  ALL API TESTS SKIPPED

From typescript/src/test/api.spec.ts (lines 13-144):

The entire test suite is marked with describe.skip() in TypeScript,
so all tests are skipped in the Python version as well.

1. ⏭️  test_hello_world (line 17)
   - Basic User CRUD operations
   - GET/POST routes

2. ⏭️  test_openapi (line 44) - ALSO SKIPPED IN TYPESCRIPT (it.skip)
   - OpenAPI schema integration
   - Auto-route generation

3. ⏭️  test_custom_scope (line 70)
   - Custom scope parameters
   - Express integration

4. ⏭️  test_exception_handling (line 117)
   - Error responses
   - 400/404 status codes

TOTAL: 4 tests (all skipped, matching TypeScript)

────────────────────────────────────────────────────────────────────────────

WHY ARE THESE TESTS SKIPPED?

The original TypeScript test file uses describe.skip() which indicates
these tests are not currently active. Possible reasons:

1. API functionality is still in development
2. Tests require additional dependencies not in core
3. Integration tests are run separately
4. Web framework integration is optional

────────────────────────────────────────────────────────────────────────────

TO ENABLE THESE TESTS:

1. Remove @pytest.mark.skip decorator from the class
2. Implement web framework integration (Flask/FastAPI)
3. Implement nucleoid.register() for class registration
4. Implement app.get(), app.post() routing methods
5. Add HTTP request testing library (httpx/requests)
...
def create_user():
    data = request.json
    user = nucleoid.run(f"new User('{data['name']}')")
    return jsonify(user)

@app.route('/users/<user_id>')
def get_user(user_id):
    user = nucleoid.run(f"User['{user_id}']")
    if user is None:
        return '', 404
    return jsonify(user)
```
...
- typescript/src/test/context.json - Test context configuration
- typescript/src/test/openapi.json - OpenAPI schema for test 2
